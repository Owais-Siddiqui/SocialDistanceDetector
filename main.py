...
def send_mail():
    msg = EmailMessage()
    msg['Subject'] = 'Over Crowding'
    msg['From'] = 'owais.iot@gmail.com'
    msg['To'] = 'owais912@gmail.com'
    msg.set_content('Potential hotspot for COVID spread detected, please act immediately.')
    ...
    with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp:
        smtp.login('owais.iot@gmail.com', 'thxteibvxuejyvhl')
        smtp.send_message(msg)
        print("email sent")


def calculate_social_distancing(vid_path, net, output_dir, output_vid, ln1, warn, crowd):
    
    count = 0
    # change 0 to vid_path to process a video.
    ...
    ####################################################################################
        ...
        # Over Crowding! send email..
        if(risk_count[0] >= 10 and risk_count[0] < 17):
           print("Please Keep Distance more, than 10 people close")
           warn = warn + 1
           #print(warn)
           if warn > 10:
               cv2.imwrite('violation.jpg',img)
               print("----------Repeated warnings given calling authorities now!!!--------------")
               send_mail()
               warn = 0
        elif(risk_count[0] >= 19):
            print("Over Crowding Alert")
            crowd+=1
            #print(crowd)
            #if crowd persistant after 5 mins then send email
            if(crowd>25):
                cv2.imwrite('violation.jpg',img)
                send_mail()
                crowd = 0
        # Best Case
            ...
            print("Great Job, Stay Safe")
        if cv2.waitKey(1) & 0xFF == ord('q'):
            #run the video maker file to convert the images to a video.
            os.system("python vidmaker.py")
            break
...
if __name__== "__main__":
    ...
    parser.add_argument('-v', '--video_path', action='store', dest='video_path', default='./data/test_video.mp4' ,
    ...
    warn = 0
    crowd = 0
    calculate_social_distancing(values.video_path, net_yl, output_dir, output_vid, ln1,warn,crowd)
